...
<!-- CI runs `pytest -m "not model"` and nothing else. The model-marked engine
     tests and scripts/api_smoke.py never run there, so if you touched the
...
- [ ] Touched the engine layer (`src/sous/engine/`): ran `uv run pytest -m model`,
      and/or `uv run python scripts/api_smoke.py`
- [ ] Touched the loopback guard (`src/sous/loopback.py`) or the forwarder
      (`src/sous/api/upstream.py`): included a test that fails without this change
