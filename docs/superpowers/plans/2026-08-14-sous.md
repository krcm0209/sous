...
- Default model: `mlx-community/Qwen3.8-27B-8bit`. Model ID is config, never hardcoded outside `config.py` defaults.
...
def test_defaults_when_file_missing(tmp_path: Path):
    ...
    assert cfg.model_id == "mlx-community/Qwen3.8-27B-8bit"
...
class SousConfig:
    ...
    model_id: str = "mlx-community/Qwen3.8-27B-8bit"
...
def load_config(config_path: Path | None = None) -> SousConfig:
    ...
    return SousConfig(
        ...
        model_id=model.get("id", "mlx-community/Qwen3.8-27B-8bit"),
...
First delegation downloads the model (~29.5 GB for the default) — one time.
...
(e.g. `mlx-community/Qwen3.8-27B-mxfp8` once published, or a fast MoE coder).
